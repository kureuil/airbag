	...
	def launch(self):
		...
		for test in self.tests:
			...
			if status is ExitStatus.ok:
				continue
			elif status in (ExitStatus.killed, ExitStatus.timeout, ExitStatus.noexec):
				failures += 1
			else:
				errors += 1
		...
		print('[{0:.0f}%] Success: {1} | Errors: {3} | Failures: {2}'.format(percentage, success, failures, errors))
		return failures